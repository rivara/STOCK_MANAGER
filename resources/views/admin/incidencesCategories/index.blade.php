...
@can('incidences_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.incidences-categories.create') }}">
                        ...
                        <tr data-entry-id="{{ $incidencesCategory->id }}">
                            ...
                            <td>
                                @can('incidences_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.incidences-categories.show', $incidencesCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('incidences_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.incidences-categories.edit', $incidencesCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('incidences_category_delete')
                                    <form action="{{ route('admin.incidences-categories.destroy', $incidencesCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('incidences_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.incidences-categories.massDestroy') }}",
