...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.incidences-categories.update", [$incidencesCategory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="incidence_category">{{ trans('cruds.incidencesCategory.fields.incidence_category') }}</label>
                <input class="form-control {{ $errors->has('incidence_category') ? 'is-invalid' : '' }}" type="text" name="incidence_category" id="incidence_category" value="{{ old('incidence_category', $incidencesCategory->incidence_category) }}" required>
