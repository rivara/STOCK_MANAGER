...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.calculations.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="calculation">{{ trans('cruds.calculation.fields.calculation') }}</label>
                <input class="form-control {{ $errors->has('calculation') ? 'is-invalid' : '' }}" type="text" name="calculation" id="calculation" value="{{ old('calculation', '') }}" required>
