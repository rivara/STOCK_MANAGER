...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.magnitude.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.magnitudes.update", [$magnitude->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="idmagnitude">{{ trans('cruds.magnitude.fields.idmagnitude') }}</label>
                <input class="form-control {{ $errors->has('idmagnitude') ? 'is-invalid' : '' }}" type="number" name="idmagnitude" id="idmagnitude" value="{{ old('idmagnitude', $magnitude->idmagnitude) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('magnitude') ? 'is-invalid' : '' }}" type="text" name="magnitude" id="magnitude" value="{{ old('magnitude', $magnitude->magnitude) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $magnitude->description) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('idunit') ? 'is-invalid' : '' }}" name="idunit_id" id="idunit_id">
                    @foreach($idunits as $id => $idunit)
                        <option value="{{ $id }}" {{ (old('idunit_id') ? old('idunit_id') : $magnitude->idunit->id ?? '') == $id ? 'selected' : '' }}>{{ $idunit }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('high') ? 'is-invalid' : '' }}" type="number" name="high" id="high" value="{{ old('high', $magnitude->high) }}" step="1">
                @if($errors->has('high'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('low') ? 'is-invalid' : '' }}" type="number" name="low" id="low" value="{{ old('low', $magnitude->low) }}" step="1">
                @if($errors->has('low'))
