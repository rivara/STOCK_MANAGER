    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.municipalities.update", [$municipality->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('province') ? 'is-invalid' : '' }}" name="province_id" id="province_id" required>
                    @foreach($provinces as $id => $province)
                        <option value="{{ $id }}" {{ (old('province_id') ? old('province_id') : $municipality->province->id ?? '') == $id ? 'selected' : '' }}>{{ $province }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('municipality') ? 'is-invalid' : '' }}" type="text" name="municipality" id="municipality" value="{{ old('municipality', $municipality->municipality) }}" required>
