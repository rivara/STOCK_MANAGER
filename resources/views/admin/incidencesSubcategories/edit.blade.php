...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.incidencesSubcategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.incidences-subcategories.update", [$incidencesSubcategory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('incidence_category') ? 'is-invalid' : '' }}" name="incidence_category_id" id="incidence_category_id" required>
                    @foreach($incidence_categories as $id => $incidence_category)
                        <option value="{{ $id }}" {{ (old('incidence_category_id') ? old('incidence_category_id') : $incidencesSubcategory->incidence_category->id ?? '') == $id ? 'selected' : '' }}>{{ $incidence_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('incidence_subcategory') ? 'is-invalid' : '' }}" type="text" name="incidence_subcategory" id="incidence_subcategory" value="{{ old('incidence_subcategory', $incidencesSubcategory->incidence_subcategory) }}" required>
