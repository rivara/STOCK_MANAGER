        ...
        <form method="POST" action="{{ route("admin.tasks.update", [$task->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('asset_status') ? 'is-invalid' : '' }}" name="asset_status_id" id="asset_status_id" required>
                    @foreach($asset_statuses as $id => $asset_status)
                        <option value="{{ $id }}" {{ (old('asset_status_id') ? old('asset_status_id') : $task->asset_status->id ?? '') == $id ? 'selected' : '' }}>{{ $asset_status }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('period') ? 'is-invalid' : '' }}" name="period_id" id="period_id">
                    @foreach($periods as $id => $period)
                        <option value="{{ $id }}" {{ (old('period_id') ? old('period_id') : $task->period->id ?? '') == $id ? 'selected' : '' }}>{{ $period }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('incidence_category') ? 'is-invalid' : '' }}" name="incidence_category_id" id="incidence_category_id">
                    @foreach($incidence_categories as $id => $incidence_category)
                        <option value="{{ $id }}" {{ (old('incidence_category_id') ? old('incidence_category_id') : $task->incidence_category->id ?? '') == $id ? 'selected' : '' }}>{{ $incidence_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('incidence_subcategory') ? 'is-invalid' : '' }}" name="incidence_subcategory_id" id="incidence_subcategory_id">
                    @foreach($incidence_subcategories as $id => $incidence_subcategory)
                        <option value="{{ $id }}" {{ (old('incidence_subcategory_id') ? old('incidence_subcategory_id') : $task->incidence_subcategory->id ?? '') == $id ? 'selected' : '' }}>{{ $incidence_subcategory }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('lost_data') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="lost_data" id="lost_data" value="1" {{ $task->lost_data || old('lost_data', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('record') ? 'is-invalid' : '' }}" name="record_id" id="record_id">
                    @foreach($records as $id => $record)
                        <option value="{{ $id }}" {{ (old('record_id') ? old('record_id') : $task->record->id ?? '') == $id ? 'selected' : '' }}>{{ $record }}</option>
                    @endforeach
                </select>
