...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="subcategory_id">{{ trans('cruds.asset.fields.subcategory') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.subcategory_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('regulations') ? 'is-invalid' : '' }}" type="text" name="regulations" id="regulations" value="{{ old('regulations', '') }}">
            ...
            <div class="form-group">
                <label class="required"  for="mark_id">{{ trans('cruds.asset.fields.mark') }}</label>
                <select class="form-control select2 {{ $errors->has('mark') ? 'is-invalid' : '' }}" name="mark_id" id="mark_id">
                    @foreach($marks as $id => $mark)
                        <option value="{{ $id }}" {{ old('mark_id') == $id ? 'selected' : '' }}>{{ $mark }}</option>
                    @endforeach
                </select>
                @if($errors->has('mark'))
            ...
            <div class="form-group">
                <label class="required"  for="sample_id">{{ trans('cruds.asset.fields.sample') }}</label>
                <select class="form-control select2 {{ $errors->has('sample') ? 'is-invalid' : '' }}" name="sample_id" id="sample_id">
                    @foreach($samples as $id => $sample)
                        <option value="{{ $id }}" {{ old('sample_id') == $id ? 'selected' : '' }}>{{ $sample }}</option>
                    @endforeach
                </select>
