    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.jobs.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="idjob">{{ trans('cruds.job.fields.idjob') }}</label>
                <input class="form-control {{ $errors->has('idjob') ? 'is-invalid' : '' }}" type="number" name="idjob" id="idjob" value="{{ old('idjob', '') }}" step="1" required>
                @if($errors->has('idjob'))
                    <span class="text-danger">{{ $errors->first('idjob') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.job.fields.idjob_helper') }}</span>
            </div>
            <div class="form-group">
