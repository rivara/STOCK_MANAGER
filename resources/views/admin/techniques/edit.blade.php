...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.technique.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.techniques.update", [$technique->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="idtechnique">{{ trans('cruds.technique.fields.idtechnique') }}</label>
                <input class="form-control {{ $errors->has('idtechnique') ? 'is-invalid' : '' }}" type="number" name="idtechnique" id="idtechnique" value="{{ old('idtechnique', $technique->idtechnique) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('technique') ? 'is-invalid' : '' }}" type="text" name="technique" id="technique" value="{{ old('technique', $technique->technique) }}" required>
