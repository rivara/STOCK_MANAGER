    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tasks.update", [$task->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ubicacion') ? 'is-invalid' : '' }}" name="ubicacion_id" id="ubicacion_id" required>
                    @foreach($ubicacions as $id => $ubicacion)
                        <option value="{{ $id }}" {{ (old('ubicacion_id') ? old('ubicacion_id') : $task->ubicacion->id ?? '') == $id ? 'selected' : '' }}>{{ $ubicacion }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('equipo') ? 'is-invalid' : '' }}" name="equipo_id" id="equipo_id" required>
                    @foreach($equipos as $id => $equipo)
                        <option value="{{ $id }}" {{ (old('equipo_id') ? old('equipo_id') : $task->equipo->id ?? '') == $id ? 'selected' : '' }}>{{ $equipo }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('estado_equipo') ? 'is-invalid' : '' }}" name="estado_equipo_id" id="estado_equipo_id" required>
                    @foreach($estado_equipos as $id => $estado_equipo)
                        <option value="{{ $id }}" {{ (old('estado_equipo_id') ? old('estado_equipo_id') : $task->estado_equipo->id ?? '') == $id ? 'selected' : '' }}>{{ $estado_equipo }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('periodicidad') ? 'is-invalid' : '' }}" name="periodicidad_id" id="periodicidad_id" required>
                    @foreach($periodicidads as $id => $periodicidad)
                        <option value="{{ $id }}" {{ (old('periodicidad_id') ? old('periodicidad_id') : $task->periodicidad->id ?? '') == $id ? 'selected' : '' }}>{{ $periodicidad }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tipo_incidencia') ? 'is-invalid' : '' }}" name="tipo_incidencia_id" id="tipo_incidencia_id">
                    @foreach($tipo_incidencias as $id => $tipo_incidencia)
                        <option value="{{ $id }}" {{ (old('tipo_incidencia_id') ? old('tipo_incidencia_id') : $task->tipo_incidencia->id ?? '') == $id ? 'selected' : '' }}>{{ $tipo_incidencia }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subtipo_incidencia') ? 'is-invalid' : '' }}" name="subtipo_incidencia_id" id="subtipo_incidencia_id">
                    @foreach($subtipo_incidencias as $id => $subtipo_incidencia)
                        <option value="{{ $id }}" {{ (old('subtipo_incidencia_id') ? old('subtipo_incidencia_id') : $task->subtipo_incidencia->id ?? '') == $id ? 'selected' : '' }}>{{ $subtipo_incidencia }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_inicio') ? 'is-invalid' : '' }}" type="text" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio', $task->fecha_inicio) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_fin') ? 'is-invalid' : '' }}" type="text" name="fecha_fin" id="fecha_fin" value="{{ old('fecha_fin', $task->fecha_fin) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('perdida_datos') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="perdida_datos" id="perdida_datos" value="1" {{ $task->perdida_datos || old('perdida_datos', 0) === 1 ? 'checked' : '' }}>
