        ...
        <div class="col-md-12">
            @can('asset_sub_category_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.asset-sub-categories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $assetSubCategory->id }}">
                                        ...
                                        <td>
                                            @can('asset_sub_category_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.asset-sub-categories.show', $assetSubCategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('asset_sub_category_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.asset-sub-categories.edit', $assetSubCategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('asset_sub_category_delete')
                                                <form action="{{ route('frontend.asset-sub-categories.destroy', $assetSubCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('asset_sub_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.asset-sub-categories.massDestroy') }}",
