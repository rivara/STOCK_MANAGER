    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.records.update", [$record->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="description">{{ trans('cruds.record.fields.description') }}</label>
                            <input class="form-control" type="text" name="description" id="description" value="{{ old('description', $record->description) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="url" id="url" value="{{ old('url', $record->url) }}">
                            @if($errors->has('url'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('url') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            @foreach(App\Models\Record::STATUS_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $record->status) === (string) $key ? 'checked' : '' }}>
