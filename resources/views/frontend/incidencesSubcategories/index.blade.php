        ...
        <div class="col-md-12">
            @can('incidences_subcategory_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.incidences-subcategories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $incidencesSubcategory->id }}">
                                        ...
                                        <td>
                                            @can('incidences_subcategory_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.incidences-subcategories.show', $incidencesSubcategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('incidences_subcategory_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.incidences-subcategories.edit', $incidencesSubcategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('incidences_subcategory_delete')
                                                <form action="{{ route('frontend.incidences-subcategories.destroy', $incidencesSubcategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('incidences_subcategory_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.incidences-subcategories.massDestroy') }}",
